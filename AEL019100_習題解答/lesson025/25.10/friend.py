...
# 用電話搜尋
def find_by_tel(file, tel_to_search):
    file.seek(0, 0)
    for line in file:
        name, gender, age, tel, addr = line.split()
        if tel == tel_to_search:
            friend_data = [name, gender, age, tel, addr]
            return friend_data
...
def clear(file):
    file.close()
    file = open(DATA_FILE, 'w+t')   # 用w模式開啟檔案，清除檔案內容
    return file   # 傳回新的檔案物件
